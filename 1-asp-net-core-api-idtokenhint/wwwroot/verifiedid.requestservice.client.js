function RequestService(
  ...
  this.createRequest = async function (url) {
    const response = await fetch(url, {
      method: "GET",
      headers: { Accept: "application/json", rsid: this.uuid },
    });
    ...
    if (respJson.error_description) {
      ...
      if (
        ...
        this.log(
          `Mobile device (${navigator.userAgent})! Using deep link (${respJson.url}).`
        );
  ...
  this.pollRequestStatus = function (id) {
    ...
    var pollFlag = setInterval(async function () {
      ...
      if (tmNow - 10 > _rsThis.request.expiry) {
        ...
        _rsThis.onError({
          error: "timeout",
          error_description: `The ${_rsThis.requestType} request was not process in time.`,
        });
        return;
      }
      const response = await fetch(
        `${_rsThis.apiPollPresentationRequest}?id=${id}`
      );
      ...
      if (respMsg.error_description) {
        ...
        if (
          respMsg.status == "presentation_error" ||
          respMsg.status == "issuance_error"
        ) {
          clearInterval(pollFlag);
          _rsThis.log(`onError(...)`);
