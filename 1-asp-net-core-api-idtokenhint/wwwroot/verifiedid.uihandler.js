...
function renderQRCode(url) {
  ...
  document
    ...
    .getElementsByTagName("img")[0].style.opacity = "1.0";
  qrcode.makeCode(url);
}
function dimQRCode() {
  document
...
function drawQRCode(requestType, id, url, pinCode) {
  ...
  if (requestType == "presentation") {
    ...
    if (pinCode != undefined) {
      ...
      document.getElementById("pinCode").style.display = "flex";
    }
...
function requestRetrieved(requestType) {
  ...
  if (requestType == "presentation") {
    displayMessage(
      "QR code scanned. Waiting for Verified ID credential to be shared from wallet..."
    );
  } else {
    displayMessage(
      "QR code scanned. Waiting for Verified ID credential to be added to wallet..."
    );
  }
}
function presentationVerified(id, response) {
  hideQRCode();
  displayMessage(
    "Presentation verified: <br/><br/>" + JSON.stringify(response.claims)
  );
...
function issuanceComplete(id, response) {
  ...
  sessionStorage.setItem("issuance_status", "issuanceComplete");
...
function setUserPhoto() {
  if (
    "none" != document.getElementById("selfie").style.display &&
    document.getElementById("selfie").src != ""
  ) {
    photoId = requestService.setUserPhoto(
      document.getElementById("selfie").src
    );
  }
}
  ...
  if (e.target.files) {
    ...
    reader.onloadend = function (e) {
      ...
      imageObj.onload = function (ev) {
        ...
        canvas.height = 640;
        console.log(
          `img size: ${imageObj.naturalWidth} x ${imageObj.naturalHeight}`
        );
        canvas
          .getContext("2d")
          .drawImage(
